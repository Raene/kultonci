...
const user = {
  ...
  actions: {
    ...
    getAddress(context, payload) {
      const token = localStorage.getItem("token");
      return axios.get(url + "/address/" + payload, { headers: { Authorization: `Bearer ${token}` } })
        .then((data) => {
          console.log("address data: ", data.data.data);
          context.commit("SET_ADDRESS",	data.data.data[0]);
    ...
    updateAddress(context, payload) {
      const token = localStorage.getItem("token");
      return axios.post(url + "/address/updtae", payload, { headers: { Authorization: `Bearer ${token}` } });
    }
  },
  getters: {
